...
public class OptionsValueController {
	...
	public String displayOptions(Model model, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
	...
	public String displayOption(HttpServletRequest request, HttpServletResponse response, Model model, Locale locale)
			throws Exception {
		return displayOption(null, request, response, model, locale);
	}

	private String displayOption(Long optionId, HttpServletRequest request, HttpServletResponse response, Model model,
	...
	public String saveOption(@Valid @ModelAttribute("optionValue") ProductOptionValue optionValue, BindingResult result,
		...
		if (optionValue.getId() != null && optionValue.getId() > 0) { // edit
																		// entry
...
*/
	...
	public @ResponseBody ResponseEntity<String> pageOptions(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (!StringUtils.isBlank(optionName)) {
				...
				// productOptionValueService.getByName(store, optionName,
				// language);
			...
			for (ProductOptionValue option : options) {
				...
				// entry.put("image", new
				// StringBuilder().append(store.getCode()).append("/").append(FileContentType.PROPERTY.name()).append("/").append(option.getProductOptionValueImage()).toString());
				entry.put("image",
	...
	public @ResponseBody ResponseEntity<String> deleteOptionValue(HttpServletRequest request,
