	...
	public String buildStaticFileAbsolutePath(MerchantStore store, String fileName) {
		...
		if(!StringUtils.isBlank(imageUtils.getBasePath()) &&
			imageUtils.getBasePath().startsWith(Constants.HTTP_SCHEME)) {
				...
				filePath.append(imageUtils.getBasePath()).append(Constants.FILES_URI).append(Constants.SLASH).append(store.getCode()).append(Constants.SLASH).append(Constants.SLASH).append(fileName).toString();
