...
/**
 * Renders a given category page based on friendly url Can also filter by facets
...
public class ShoppingCategoryController {
	...
	private String displayCategory(final String friendlyUrl, final String ref, Model model, HttpServletRequest request,
		...
		subCategoriesCacheKey.append(store.getId()).append("_").append(category.getId()).append("_")
				.append(Constants.SUBCATEGORIES_CACHE_KEY).append("-").append(language.getCode());
		...
		if (store.isUseCache()) {
			...
			if (subCategories == null) {
				...
				// (Boolean)cache.getFromCache(subCategoriesMissed.toString());
				...
				if (subCategories != null) {
					...
					// subCategoriesCacheKey.toString());
				}
				// }
			}
		} else {
		...
		/** template **/
		StringBuilder template = new StringBuilder().append(ControllerConstants.Tiles.Category.category).append(".")
	...
	private List<ReadableManufacturer> getManufacturersByProductAndCategory(MerchantStore store, Category category,
		...
		if (subCategoryIds != null && subCategoryIds.size() > 0) {
			...
			manufacturersKey.append(store.getId()).append("_").append(Constants.MANUFACTURERS_BY_PRODUCTS_CACHE_KEY)
					.append("-").append(language.getCode());

			StringBuilder manufacturersKeyMissed = new StringBuilder();
			manufacturersKeyMissed.append(manufacturersKey.toString()).append(Constants.MISSED_CACHE_KEY);
			...
			if (store.isUseCache()) {
				...
				if (manufacturerList == null) {
					// get from missed cache
					// Boolean missedContent =
					// (Boolean)cache.getFromCache(manufacturersKeyMissed.toString());
					// if(missedContent==null) {
					...
					if (manufacturerList.isEmpty()) {
						...
						// cache.putInCache(manufacturerList,
						// manufacturersKey.toString());
					}
					// }
				}
			} else {
	...
	private List<ReadableManufacturer> getManufacturers(MerchantStore store, List<Long> ids, Language language)
		...
		if (!manufacturers.isEmpty()) {
			...
			for (co.hooghly.commerce.domain.Manufacturer manufacturer : manufacturers) {
				ReadableManufacturer manuf = new ReadableManufacturerPopulator().populate(manufacturer,
						new ReadableManufacturer(), store, language);
	...
	private List<ReadableCategory> getSubCategories(MerchantStore store, Category category,
		...
		for (Category sub : subCategories) {
			...
			// com.salesmanager.web.entity.catalog.Category cProxy =
			// catalogUtils.buildProxyCategory(sub, store, locale);
	...
	@ResponseBody
	public List<ReadableCategory> getCategories(@PathVariable final String language, @PathVariable final String store,
		...
		if (merchantStore == null) {
			...
			response.sendError(503, "Merchant store is null for code " + store);// TODO
																				// localized
																				// message
			return null;
		}
	...
	/**
	 * Returns an array of products belonging to a given category in a given
	 * language for a given store url example :
	 * http://<host>/sm-shop/shop/services/public/products/DEFAULT/BOOKS
	 * 
	 * @param store
	 * @param language
	 * @param category
	...
	@ResponseBody
	public ProductList getProducts(@PathVariable final String store, @PathVariable final String language,
			@PathVariable final String category, Model model, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		...
		try {
			...
			if (cat == null) {
				...
				response.sendError(503, "Category is null");// TODO localized
															// message
			}
			...
			List<co.hooghly.commerce.domain.Product> products = productService.getProducts(ids, lang);
	...
	@ResponseBody
	public ProductList getProducts(@PathVariable int start, @PathVariable int max, @PathVariable String store,
	...
	public ProductList getProductsFilteredByType(@PathVariable int start, @PathVariable int max,
			@PathVariable String store, @PathVariable final String language, @PathVariable final String category,
			@PathVariable final String filterType, @PathVariable final String filterValue, Model model,
			HttpServletRequest request, HttpServletResponse response) throws Exception {

		List<QueryFilter> queryFilters = null;
		try {
			if (filterType.equals(QueryFilterType.BRAND.name())) {// the only
																	// one
																	// implemented
																	// so far
	...
	private ProductList getProducts(final int start, final int max, final String store, final String language,
			final String category, final List<QueryFilter> filters, Model model, HttpServletRequest request,
		...
		try {
			...
			if (filters != null) {
				for (QueryFilter filter : filters) {
					if (filter.getFilterType().name().equals(QueryFilterType.BRAND.name())) {// the
																								// only
																								// filter
																								// implemented
