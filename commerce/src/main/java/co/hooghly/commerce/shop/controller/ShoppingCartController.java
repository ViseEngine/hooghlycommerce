...
/**
 * A mini shopping cart is available on the public shopping section from the
 * upper menu Landing page, Category page (list of products) and Product details
 * page contains a form that let the user add an item to the cart, see the
 * quantity of items, total price of items in the cart and remove items
 *
 * Add To Cart --------------- The add to cart is 100% driven by javascript /
 * ajax. The code is available in webapp\resources\js\functions.js
 *
 * <!-- Simple add to cart html example ${id} is the product id -->
 * <form id="input-${id}">
 * <input type="text" class="input-small" id="quantity-productId-${id}"
 * placeholder="1" value="1">
 * <a href="#" class="addToCart" productId="${id}">Add to cart</a> </form>
 *
 * The javascript function creates
 * com.salesmanager.web.entity.shoppingcart.ShoppingCartItem and
 * ShoppingCartAttribute based on user selection The javascript looks in the
 * cookie if a shopping cart code exists ex $.cookie( 'cart' ); // requires
 * jQuery-cookie The javascript posts the ShoppingCartItem and the shopping cart
 * code if present to /shop/addShoppingCartItem.html
 *
 * @see
 *
 * 		javascript re-creates the shopping cart div item (div id shoppingcart)
 *      (see webapp\pages\shop\templates\bootstrap\sections\header.jsp) The
 *      javascript set the shopping cart code in the cookie
 *
 *      Display a page ----------------
 *
 *      When a page is displayed from the shopping section, the shopping cart
 *      has to be displayed 4 paths 1) No shopping cart 2) A shopping cart exist
 *      in the session 3) A shopping cart code exists in the cookie 4) A
 *      customer is logeed in and a shopping cart exists in the database
 *
 *      1) No shopping cart, nothing to do !
 *
 *      2) StoreFilter will tak care of a ShoppingCart present in the
 *      HttpSession
 *
 *      3) Once a page is displayed and no cart returned from the controller, a
 *      javascript looks on load in the cookie to see if a shopping cart code is
 *      present If a code is present, by ajax the cart is loaded and displayed
 *
 *      4) No cart in the session but the customer logs in, the system looks in
 *      the DB if a shopping cart exists, if so it is putted in the session so
 *      the StoreFilter can manage it and putted in the request
 */
...
public class ShoppingCartController extends AbstractController {
	...
	public @ResponseBody ShoppingCartData addShoppingCartItem(@RequestBody final ShoppingCartItem item,
		...
		if (customer != null) {
			co.hooghly.commerce.domain.ShoppingCart customerCart = shoppingCartService
	...
	public String displayShoppingCart(final Model model, final HttpServletRequest request,
		...
		if (StringUtils.isBlank(cartCode)) {
			// display empty cart
			StringBuilder template = new StringBuilder().append(ControllerConstants.Tiles.ShoppingCart.shoppingCart)
	...
	@GetMapping("/{shoppingCartByCode}")
	public String displayShoppingCart(@PathVariable("shoppingCartByCode") String shoppingCartCode, final Model model,
			HttpServletRequest request, HttpServletResponse response, final Locale locale) throws Exception {
		
		log.info("== We are here ==" + shoppingCartCode);
	...
	String removeShoppingCartItem(final Long lineItemId, final HttpServletRequest request,
		...
		// ** need to check if the item has property, similar items may exist
		// but with different properties
		// String attributes = request.getParameter("attribute");//attributes id
		// are sent as 1|2|5|
		...
		ShoppingCartData shoppingCartData = shoppingCartFacade.removeCartItem(lineItemId, shoppingCart.getCode(), store,
				language);
	...
	public @ResponseBody String updateShoppingCartItem(@RequestBody final ShoppingCartItem[] shoppingCartItems,
