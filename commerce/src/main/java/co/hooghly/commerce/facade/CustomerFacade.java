...
import co.hooghly.commerce.web.populator.CustomerBillingAddressPopulator;
import co.hooghly.commerce.web.populator.CustomerDeliveryAddressPopulator;
import co.hooghly.commerce.web.populator.CustomerEntityPopulator;
import co.hooghly.commerce.web.populator.CustomerPopulator;
import co.hooghly.commerce.web.populator.PersistableCustomerBillingAddressPopulator;
import co.hooghly.commerce.web.populator.PersistableCustomerShippingAddressPopulator;
...
public class CustomerFacade {
	...
	public CustomerEntity getCustomerDataByUserName(final String userName, final MerchantStore store,
		...
		if (customer != null) {
			...
			try {
				...
				return customerEntityPopulator.populate(customer, store, language); // store,
																					// language
			...
			if (StringUtils.isNotBlank(sessionShoppingCartId)) {
				...
				if (sessionShoppingCart != null) {
					if (customerCart == null) {
						if (sessionShoppingCart.getCustomerId() == null) {// saved
																			// shopping
																			// cart
																			// does
																			// not
	...
	/**
	 * <p>
	 * Method to check if given user exists for given username under given
	 * store. System treat username as unique for a given store, customer is not
	 * allowed to use same username twice for a given store, however it can be
	 * used for different stores.
	 * </p>
	 * 
	 * @param userName
	 *            Customer slected userName
	 * @param store
	 *            store for which customer want to register
	...
	public CustomerEntity registerCustomer(final PersistableCustomer customer, final MerchantStore merchantStore,
		...
		if (customerModel == null) {
			...
			// throw new CustomerRegistrationException( "Unable to register
			// customer" );
	...
	public Address getAddress(Long userId, final MerchantStore merchantStore, boolean isBillingAddress)
		...
		if (customerModel == null) {
			...
			// throw new CustomerNotFoundException( "customer with given id does
			// not exists" );
