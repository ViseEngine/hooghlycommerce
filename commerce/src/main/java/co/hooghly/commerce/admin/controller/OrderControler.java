...
private static final Logger LOGGER = LoggerFactory.getLogger(OrderControler.class);
	...
	private String displayOrder(Long orderId, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		co.hooghly.commerce.domain.admin.Order order = new co.hooghly.commerce.domain.admin.Order();
	...
	public String saveOrder(@Valid @ModelAttribute("order") co.hooghly.commerce.domain.admin.Order entityOrder, BindingResult result, Model model, HttpServletRequest request, Locale locale) throws Exception {
		...
		co.hooghly.commerce.domain.Order newOrder = orderService.getById(entityOrder.getOrder().getId() );
