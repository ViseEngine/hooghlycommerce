...
public class CategoryController {
	...
	private Map convert(Category category) {
		 ...
		 * 
		 * entry.put("title", description.getName()); entry.put("key",
		 * category.getId() + "");
		 * 
		 * List<Category> children = category.getCategories(); // look only 1
		 * level down for children if (children != null && !children.isEmpty())
		 * { List<Map> childrenList = new ArrayList<Map>(); for (Category c :
		 * children) { Map childEntry = new HashMap();
		 * 
		 * CategoryDescription childDescription = c.getDescriptions().get(0);
		 * 
		 * childEntry.put("title", childDescription.getName());
		 * childEntry.put("key", c.getId() + ""); childEntry.put("lazy", true);
		 * 
		 * childrenList.add(childEntry); }
		 * 
		 * entry.put("children", childrenList); } else { entry.put("lazy",
		 * false); entry.put("children", null); }
		 */
		return entry;
	}
	...
	public @ResponseBody ResponseEntity<String> pageCategories(HttpServletRequest request,
		...
		try {
			...
			for (Category category : categories) {
				...
				// category.getDescriptions().get(0);
