	...
	function initMiniCart() {
		...
		//alert('Check for cart code ' + cartCode);
		if(cartCode!=null) {
			miniCart(cartCode);
		} else {
			//display empty cart
	...
	function addToCart(sku) {
		...
		$.ajax({  
			 type: 'POST',  
			 url: '/shop/cart/addShoppingCartItem',  
			 data: scItem, 
...
function miniCart(cartCode){
	...
	//alert('Display cart content - ' + cartCode);


	$.ajax({  
		 type: 'GET',  
		 url: '/shop/cart/mini/'+cartCode,  
		 cache:false,
		 error: function(e) { 
			 //alert('error ' + e);
			 //nothing
...
function miniCartSummary(cart) {
    ...
    alert('miniCartSummaryRendered - ' + miniCartSummaryRendered);
    
    alert('element - ' + $('#miniCartSummary'));
   
    $('#miniCartSummaryDisplay').html(miniCartSummaryRendered);
    
   
    //update counter
    $('.input-small').val(cart.quantity == 0 ? 1 : cart.quantity);
...
function buildCartCode(code) {
	var cartCode = merchatStoreCode + '_' + code;
	return cartCode;
}
